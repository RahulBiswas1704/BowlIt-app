...
export default function ProfilePage() {
  ...
  return (
    <main className="fixed inset-0 h-[100dvh] bg-gray-50 flex justify-center overflow-hidden">
      ...
      <div className="w-full max-w-lg relative z-10 h-full">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'menu' && (
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              className="pt-12 px-5 space-y-8 h-full overflow-y-auto pb-24"
            >
              {/* TOP HEADER */}
          ...
          {activeTab !== 'menu' && (
            <motion.div
              ...
              className="absolute inset-0 z-40 bg-gray-50 h-[100dvh] w-full flex flex-col"
            >
              {/* FIXED HEADER FOR SUB-VIEWS */}
              <div className="flex-shrink-0 flex items-center gap-4 px-5 py-4 bg-white border-b border-gray-100 z-50 shadow-sm relative">
              ...
              <div className="flex-1 overflow-y-auto p-5 pb-24 space-y-6">
                ...
                {activeTab === 'orders' && (
                  <div className="space-y-5">
                    {orders.length === 0 ? (
                      <div className="text-center py-16 bg-white rounded-[2rem] shadow-sm border border-gray-100 text-gray-400 font-medium flex flex-col items-center justify-center gap-4">
                        <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center"><ShoppingBag size={24} className="text-gray-300" /></div>
                        No past deliveries found.
                      </div>
                    ) : (
                      orders.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map(order => (
                        <div key={order.id} className="group flex flex-col p-5 rounded-[1.5rem] bg-white border border-gray-100 hover:border-orange-200 transition-all gap-4 shadow-sm hover:shadow-orange-100/50">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${order.status === 'Completed' ? 'bg-green-50 text-green-600' : order.status === 'Cancelled' ? 'bg-red-50 text-red-600' : 'bg-orange-50 text-orange-600'}`}>
                                <Utensils size={20} />
                              </div>
                              <div>
                                <div className="font-black text-gray-900 text-base mb-0.5">Order #{order.id}</div>
                                <div className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">{new Date(order.created_at).toLocaleDateString('en-IN', { weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</div>
                              </div>
                            </div>
                            <span className="font-black text-gray-900 text-lg">₹{parseFloat(order.total_amount).toFixed(2)}</span>
                          </div>
                          <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                            <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${order.status === 'Completed' ? 'text-green-700 bg-green-50' : order.status === 'Cancelled' ? 'text-red-700 bg-red-50' : 'text-orange-700 bg-orange-50'}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${order.status === 'Completed' ? 'bg-green-500' : order.status === 'Cancelled' ? 'bg-red-500' : 'bg-orange-500 animate-pulse'}`}></span> {order.status}
                            </span>
                            <button className="text-[11px] font-black text-orange-600 bg-orange-50 hover:bg-orange-100 px-4 py-2 rounded-xl tracking-widest uppercase transition-colors">
                              View Receipt
                            </button>
                          </div>
                        </div>
