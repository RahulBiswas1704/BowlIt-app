...
export function LiveTrackingCard({ userId }: { userId: string }) {
    ...
    // Setup Rider Subscription if Rider is assigned
    useEffect(() => {
        if (!rider?.phone) return;

        const riderChannel = supabase
            .channel(`public:riders:phone=eq.${rider.phone}`)
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'riders', filter: `phone=eq.${rider.phone}` }, (payload) => {
